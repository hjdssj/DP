...
def style_axis(ax, dataset, show_ylabel=True):
    ax.axhline(0, color="#444444", linewidth=1.1, alpha=0.9)
...
def save_single_dataset_figures(df):
    filenames = {
        "MNIST": "fixed_dp_gain_mnist",
        "Fashion-MNIST": "fixed_dp_gain_fashion_mnist",
        "CIFAR-10": "fixed_dp_gain_cifar10",
    }
...
def save_summary_figure(df):
    ...
    save_figure(fig, "fixed_dp_gain_all_datasets")
    plt.close(fig)
...
def main():
    ...
    print("Saved fixed-DP gain figures:")
    for name in [
        "fixed_dp_gain_mnist",
        "fixed_dp_gain_fashion_mnist",
        "fixed_dp_gain_cifar10",
        "fixed_dp_gain_all_datasets",
    ]:

...
def plot_method_lines(ax, subset):
    for method in METHODS:
        method_df = subset[subset["method"] == method].sort_values("target_epsilon")
        ax.plot(
            method_df["target_epsilon"],
...
def save_single_dataset_figures(df):
    filenames = {
        "MNIST": "naive_stable_mse_lines_mnist",
        "Fashion-MNIST": "naive_stable_mse_lines_fashion_mnist",
        "CIFAR-10": "naive_stable_mse_lines_cifar10",
    }
...
def save_summary_figure(df):
    ...
    save_figure(fig, "naive_stable_mse_lines_all_datasets")
    plt.close(fig)
...
def main():
    ...
    print("Saved Naive-vs-Stable MSE line figures:")
    for name in [
        "naive_stable_mse_lines_mnist",
        "naive_stable_mse_lines_fashion_mnist",
        "naive_stable_mse_lines_cifar10",
        "naive_stable_mse_lines_all_datasets",
    ]:

...
def main():
    ...
    parser.add_argument("--hist-bin-max", type=float, default=None,
                        help="Upper range for gradient norm histogram bins; larger values reduce overflow in the last bin")
    ...
    for run_idx in range(args.n_runs):
        ...
        if args.mode in ('ratio', 'mse'):
            if args.hist_bin_max is not None:
    ...
    if args.save_model:
        model_path = RESULTS_DIR / f"fashion_resnet18_{repro_str}.pt"

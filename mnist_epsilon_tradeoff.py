...
Fixed: epochs=20, batch=256, lr=0.1, delta=1e-5
"""
...
def make_loaders(data_root):
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST(data_root, train=True, download=True, transform=tf),
        batch_size=BATCH, num_workers=0, pin_memory=True,
    )
    test_loader = torch.utils.data.DataLoader(
        datasets.MNIST(data_root, train=False, transform=tf),
...
def train_epoch_fixed(model, optimizer, train_loader, device, criterion):
    model.train()
    losses = []
...
def train_epoch_adaptive(model, optimizer, train_loader, device, criterion, controller):
    model.train()
    losses = []
    total, clipped = 0, 0
...
def run_fixed(sigma, device, data_root):
    ...
    for epoch in range(1, EPOCHS + 1):
        loss = train_epoch_fixed(model, optimizer, train_loader, device, criterion)
...
def run_adaptive(sigma, device, data_root):
    train_loader, test_loader = make_loaders(data_root)
    model = SampleConvNet().to(device)
    optimizer = optim.SGD(model.parameters(), lr=LR, momentum=0)
    criterion = nn.CrossEntropyLoss()
    ...
    for epoch in range(1, EPOCHS + 1):
        loss, clip_ratio, new_c = train_epoch_adaptive(
            model, optimizer, train_loader, device, criterion, controller)
        eps = pe.accountant.get_epsilon(delta=DELTA)
...
def main():
    ...
    parser.add_argument("--output", default="mnist_epsilon_tradeoff.csv")
    ...
    fixed_results = cache.get('fixed', {})   # sigma -> (eps, acc)
    ...
    for sigma in SIGMAS:
        ...
        if key not in fixed_results:
            ...
            eps, acc = run_fixed(sigma, device, args.data_root)
        ...
        if key not in adapt_results:
            ...
            eps, acc = run_adaptive(sigma, device, args.data_root)
            print(f"  => ε={eps:.3f}, acc={acc*100:.2f}%")
